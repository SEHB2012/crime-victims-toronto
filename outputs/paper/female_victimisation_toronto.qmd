...
R [@citeR] was the language and environment used for the bulk of this analysis, alongside the dplyr (@dplRef),tidyverse[@tidyverseRef], janitor[@janitorRef], knitr and readr packages. The following formatting packages were used for the tables, scatter plot and heatmap: kable, ggrepel, and pheatmap.
...
#| tbl-cap: "Cleaned data featuring the sum count variable"
#| echo: false
#| warning: false
#| message: false
...
## Changing female victimization rates in Toronto over the years 
...
```
Figure x is a scatterplot that visually represents the data on how victimization rates have increased over the years.
```{r}
...
The pheatmap package (an appropriate abbreviation for what was indeed a 'pretty heatmap') generated a visually appealing and customisable chart that allowed for clustering and custom labels. (see figure blah blah)
...
```{r fig.align='center'}
#| label: fig-heatmap
#| fig-cap: "Figure 3: Heatmap showing trends"
#| echo: false
#| warning: false
#| message: false
...
Bin width sizes for the age cohorts were another noticeable issue. The range was lower for the adolescent (12 to 17) and young adult(18 to 24)  groups, being just 7 years. It was 10 years for all four of the middle-aged groups. As for the fringe ages that were unbounded, <12 and >65, there is no discrete way of making sure what ages within those ranges were accounted for. The absence of bin width standardisation would mean bar charts with ages on the horizontal axis had differing bar widths, and that resulting inferences would not be truly representative of the data.

### Unreported Crime cases

Lastly, one must note that this dataset only considers victims who have come forward to the Toronto Police to file official complaints, not the actual number of crimes committed in the city. The General Social Survey conducted by Statistics Canada enables self-reporting of crime through a series of independent, annual, voluntary, cross-sectional surveys. According to the Victimization cycle of the GSS conducted in 2009, 69% of violent victimizations and 71% of robberies are not reported to the police. This means that this Victims of Crime dataset published by the Toronto Police only reflects a fraction of the crimes committed. It is also prudent to mention that the GSS has the limitation of reliance upon respondents for accurately recalling and reporting the events, but in conjunction with the Annual Statistical report, could provide a more authentic count of the crimes in Toronto.
As an aside, the GSS confers with the results of this study posits that being young remains a strong predictor of victimization against women, and that younger females are more vulnerable to violent crime. (cite)
